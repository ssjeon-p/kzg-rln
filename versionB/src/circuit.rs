...
use halo2::{
    ...
    halo2curves::bn256::{G1Affine, Fq, Fr},
    plonk::{Circuit, ConstraintSystem, Error}, dev::MockProver,
};

use ecc::{
    integer::rns::Rns,
    integer::NUMBER_OF_LOOKUP_LIMBS,
    maingate::{MainGate, MainGateConfig, RangeChip, RangeConfig, RangeInstructions, RegionCtx, AssignedValue, MainGateInstructions},
    ...
    fn synthesize(
        ...
        layouter.assign_region(
            ...
            |region| {
                ...
                ecc_chip.assign_aux(ctx, 2, self.degree + 1)?;
                Ok(())
            },
        )?;
...
fn rns_setup(
    k_override: u32,
) -> (Rns<BASE, SCALAR, NUMBER_OF_LIMBS, BIT_LEN_LIMB>, u32) {
...
pub fn create_zkp(
    ...
    let circuit = Mycircuit::<NUMBER_OF_LIMBS, BIT_LEN_LIMB> {
        degree: 3,
        coeffs: coeffs,
        setup: setup,
    };

    let (rns, _) = rns_setup(1);
    let rns = Rc::new(rns);
...
mod tests {
    ...
    use halo2::{
        dev::MockProver,
        halo2curves::{bn256::Bn256, group::Curve},
        poly::{commitment::ParamsProver, kzg::commitment::ParamsKZG, EvaluationDomain}, arithmetic::Field,
    };
    use rand::thread_rng;
    ...
    fn test_commit() {
        ...
        let commit = keys
            .commit(&poly, halo2::poly::commitment::Blind::default())
            .to_affine();
        ...
        // let random = G1Affine::random(rng);
        // let random = Point::new(Rc::clone(&rns), random);
        // let random = random.public();
        // let prover = MockProver::run(18, &test_circuit, vec![random]).unwrap();

        let input = Point::new(Rc::clone(&rns), commit);
        let input = input.public();
